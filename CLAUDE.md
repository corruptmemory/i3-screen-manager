...
- `hypr-focus` — `hypr-focus <left|right>`: i3-style group-aware directional focus. Bound to `Super+left/right` (via a `has_cmd` guard in `bindings.lua`; falls back to plain `focus({direction})` where undeployed, e.g. the laptop until symlinked). Reads the active window's group state (`hyprctl activewindow -j`, ~6ms) and dispatches exactly ONE action so the two i3 behaviors don't collide: **multi-window group → `group.prev/next`** (cycle its tabs, wraps); **else → `focus({direction})`** (move between tiled windows). Exists because Hyprland splits what i3's `focus left/right` unifies — a group is one tile to `focus({direction})`, and `group.prev/next` never leaves the group — and binding a key to BOTH fires both, giving the ws10 comms-wall "bounce" (cycles a tab AND jumps groups every press). Deliberately NOT a cross-group "walk all N": exiting a group at the boundary escapes the comms wall's lone portrait monitor; cross-group jumps stay on `Super+Ctrl+arrows`. Validated live 2026-08-31 on both test cases (grouped ws10 wall → cycle; DP-2 plain tiles → directional). **Port target:** reimplement as a Lua bind function — Hyprland's Lua objects DO expose live window state (`window`/`workspace`/`monitor` userdata, fields read via `__index`; confirmed in `/usr/include/hyprland/src/config/lua/`), so the decision logic belongs in-config; the one unknown is the window object's exact field names for the group member list.
