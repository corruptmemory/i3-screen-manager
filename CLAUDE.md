...
- **GTK file dialog hangs 25 seconds**: `gvfsd-trash` D-Bus backend times out when GTK FileChooserDialog builds its sidebar. Affects open/save dialogs in all GTK apps (Brave, Firefox, etc.). Fix: `export GIO_USE_VFS=local` in `~/.xinitrc` before `exec i3`. This tells GLib to use direct POSIX file access instead of the gvfs D-Bus backend. Only loss is `trash://` and `network://` URIs in GTK apps, which are unused on i3. Diagnosis steps: (1) `time gio info trash:///` — if this hangs ~25s, you have the bug; (2) confirm with `time GIO_USE_VFS=local gio info trash:///` — should be instant; (3) test a GTK dialog directly: `time GIO_USE_VFS=local python3 -c "import gi; gi.require_version('Gtk','3.0'); from gi.repository import Gtk; d=Gtk.FileChooserDialog(title='test',action=Gtk.FileChooserAction.OPEN); d.destroy(); print('done')"`. This issue exists on both desktop and laptop — apply the xinitrc fix to both machines.
