...
| None of `pipewire` / `wireplumber` / `pipewire-pulse` autostart | Do not assume OpenRC user services will launch the audio stack. On some boxes they aren't configured, and on others only a subset (e.g. just `pipewire`) gets started. Launch **all three** from `hyprland.conf` exec-once in order. Partial stacks are worse than missing ones: if `pipewire-pulse` is the only one running, the pulse socket appears at `/run/user/$UID/pulse/native` and accepts connections, but every protocol exchange fails with `Host is down` (no pipewire core behind it). Waybar's `pulseaudio` module retries on failure and leaks a file descriptor per attempt, exhausting `RLIMIT_NOFILE` and dying with "Too many open files" spam that taints the whole session. Cascading symptoms: `hyprland/window` module also dies ("Hyprland IPC: Couldn't open a socket"), Brave launching tips waybar over the edge because libpulse adds load, workspace indicators vanish, etc. Fix: <pre>exec-once = pipewire<br>exec-once = sleep 0.5 && wireplumber<br>exec-once = sleep 1 && pipewire-pulse<br>exec-once = sleep 2 && waybar & mako</pre> Diagnose with `pgrep -x pipewire wireplumber pipewire-pulse` (need all three) and `pactl info` (should return server info, not errors). A leftover `pipewire-pulse` from a previous session that outlived logout can also wedge the socket; `pkill -9 -x pipewire-pulse` before relaunch if `/run/user/$UID/pulse/pid` names a dead-ish process. |
...
| Brave extension popups (Bitwarden, etc.) ignore `match:title` rules | Extension popup windows map with `initialTitle = _crx_<extension-id>` then change to the human-readable name *after* mapping. Since Hyprland evaluates rules at map time, `match:title` never sees the final title. Fix: match on `match:class = ^(brave-<extension-id>.*)` instead — the class includes the extension ID from the start. The `.*` suffix is needed because Brave appends `-Profile_N`. `match:initialTitle` does not exist in 0.54.x. |
| `xdg-desktop-portal-gtk` file dialogs tiled | Browser save/open dialogs spawned via XDG portals use class `xdg-desktop-portal-gtk`, not the browser class. Add a windowrule matching this class with `float = true` and `center = true`. |
