...
| Hyprland idles hotter than i3/X11 (CPU stuck in C2/C3, won't reach C8+) | `vfr = true` missing from `misc {}` in hyprland.conf. Without it Hyprland redraws at full refresh rate (~60 wakeups/sec) even when nothing is moving, preventing deep CPU C-states. Add `vfr = true` to the misc block. Under i3/picom, the compositor only rendered on damage — VFR restores that behaviour in Hyprland. |
