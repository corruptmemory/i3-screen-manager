...
**Use the `.pacnew` as the candidate pool**, not the current (already-ranked) mirrorlist. Whenever pacman updates the `pacman-mirrorlist` package, a fresh `/etc/pacman.d/mirrorlist.pacnew` appears containing the full upstream mirror set (~60 URLs). Re-ranking against your existing top-6 file just re-sorts the same 6, which defeats the purpose.

```bash
# 1. Back up the current mirrorlist
sudo cp /etc/pacman.d/mirrorlist /etc/pacman.d/mirrorlist.bak

# 2. Speed-test all candidate mirrors against the 'world' repo (~2 min for 60 mirrors)
#    Output is verbose: every mirror with its time as a comment, then top N as Server lines.
sudo rankmirrors -v -n 10 -r world /etc/pacman.d/mirrorlist.pacnew > /tmp/ranked

# 3. Dedupe by URL and take the top 6 — the upstream pacnew lists popular mirrors
#    in multiple geographic sections, so rankmirrors emits duplicates.
grep -E '^# https?://.*\.\.\. [0-9]' /tmp/ranked \
  | sed 's/^# //; s/\.\.\. /\t/' \
  | sort -k2 -n -u \
  | awk -F'\t' '!seen[$1]++ { print "Server = " $1 }' \
  | head -6

# 4. Inspect the 6 lines; if they look reasonable, write the final mirrorlist
#    (with header + the 6 dedup'd Server lines), then remove the pacnew

# 5. Force a full database refresh against the new mirrors
sudo pacman -Syy
```

### Why `-r world` is mandatory on Artix

`rankmirrors` defaults to testing the `core` repo (the Arch convention). Artix has no `core` — its repos are `system`, `world`, `galaxy`, `lib32`. Without `-r world` every mirror 404s and the output reports them all as "unreachable". Use `world` (largest repo, most reliable indicator).

### Why dedupe matters

Artix's `mirrorlist.pacnew` is generated by `artix-mlg`, which concatenates a "Default mirrors" block at the top with a country-grouped block below. Several mirrors (e.g. `mirrors.rit.edu`) appear in both. rankmirrors tests both copies and emits both Server lines, silently wasting a slot in your top-N.

### Why `-Syy`, not `-Sy`

The double-`y` forces re-download of the package databases even when they look up-to-date. `pacman -Sy` may use cached DBs and never actually contact the new mirrors — leaving you uncertain whether they work. Always `-Syy` after a mirrorlist swap.

### Don't use `artix-mlg` to rank

`artix-mlg` is a developer tool for maintaining the *official* upstream mirror list — not for picking your local fastest mirrors. Use `rankmirrors` from `pacman-contrib` instead.
