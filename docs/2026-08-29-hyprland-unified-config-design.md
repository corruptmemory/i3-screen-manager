...
## 11. Update (2026-08-29): `browserAltProfileDir` added as a per-machine constant

`Super+y` in `bindings.lua` launches Brave-Origin with a
`--profile-directory` for the secondary account (pennystinker@gmail). The
directory name is auto-generated by Brave at profile-creation time and
happens to differ between the two machines: **`Profile 3` on the desktop,
`Profile 1` on the laptop.** Ground truth is Brave's own
`~/.config/BraveSoftware/Brave-Origin/Local State` (walk
`profile.info_cache` — directory keys map to `name` / `gaia_name`).

Fix followed the record-field pattern: new `browserAltProfileDir` on each
`HOSTS` row (and on the unknown-host fallback), and `vars.lua` interpolates
`m.browserAltProfileDir` into the `browserAlt` command string.
`bindings.lua` was left alone (already reads `v.browserAlt`); only its
description text changed from "Profile 3 / pennystinker" (misleading on
laptop) to "secondary profile / pennystinker@gmail".

Field taxonomy this shipped under (extending § 4's Notes):

- **Compatibility-bridge selectors** (`bar`, `screenshot`) — tools in
  transition; a machine graduates by flipping the value. Snapshot values
  are current-state, not permanent.
- **Static per-machine constants** (`fileManager`, `browserAltProfileDir`)
  — permanent facts about how each machine was installed. Snapshot values
  are the intended long-term values.

`browserAltProfileDir` is the second member of the second class. The
distinction matters for reader expectations: nobody should read
`fileManager = "pcmanfm"` and think "the desktop is *transitioning to*
pcmanfm."

Test surface: `test_vars.lua` now asserts BOTH machines resolve to their
own `--profile-directory=` value (was only desktop before). Snapshot of
§ 4's record left as-is (post-2026-08-29 field additions are recorded here
rather than back-editing the point-in-time example).
