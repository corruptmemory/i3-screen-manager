...
### Round-3 — click-to-focus, focus-fallback helper, GNOME maximize (2026-06-16)

Daily-use refinements after living in the session.

- **Focus model → click-to-focus + raise.** Swapped `mouse` to
  `INCLUDE mouse_click` (the empty file — it just omits the `Enter→Focus` bindings
  that `mouse_sloppy` adds) and set `$CLIENT_CLICK = "Focus; Raise"`. In a stacking
  WM the focused window should also be the topmost one, so a body click focuses
  *and* raises; hovering no longer steals focus.
  - **Gotcha:** pekwm 0.4.4's in-session `Reload` *and* `Restart` do **not** re-read
    the **mouse** config — the model swap only takes effect on a fresh session start.
    Verified the live session stayed sloppy after both, while keys/theme reloaded fine
    and the `~/.pekwm/mouse` Super+right-drag resize override was demonstrably live
    (so the file *is* loaded — only the focus model is frozen until respawn). Don't
    chase this with `Reload`; log out/in.

- **Focus-fallback after `SendToWorkspace` (the "no window focused" bug).** Sending
  the focused window to another workspace left the *source* workspace with nothing
  focused. Root cause is **independent of the focus model** — pekwm simply assigns no
  fallback focus after `SendToWorkspace` (click-to-focus did NOT fix it). Appending
  `NextFrame*`/`PrevFrame*` to the keybind re-grabs the just-sent window (the frame
  cycle races the move). **Fix:** a helper, `.local/bin/pekwm-send-to-ws <N>`, that
  captures the topmost *remaining* window on the current workspace *before* the move
  (EWMH `_NET_CLIENT_LIST_STACKING`), fires `SendToWorkspace`, then `windowactivate`s
  that survivor. Topmost-remaining = the previously-focused window under
  click-to-focus+raise. All ten `Mod4 Shift N` binds route through it; plain
  `SendToWorkspace N` stays on the Polybar workspace binds.

- **GNOME-style maximize.** `Mod4 Up → Set Maximized True True`,
  `Mod4 Down → Unset Maximized True True` (Set/Unset, not Toggle, so Up always
  maximizes / Down always restores). `Maximized` honors the Polybar strut
  (2556×1399, bar visible) — unlike `Toggle FullScreen`. Replaces `FocusDirectional
  Up/Down`; Left/Right directional focus stays, up/down reach via `Mod4 Tab` (rofi)
  / `Mod1 Tab` (MRU).

- **Gotcha: pekwm `Exec` does NOT expand `~`.** The helper silently never ran until
  the bind used an absolute path — and the same bug had been silently breaking
  `Mod4 Shift W` (Polybar relaunch, `Exec ~/.config/polybar/launch.sh`) all along.
  pekwm appears to `execvp` Exec strings rather than route them through `sh -c`, so
  the tilde stays literal. Use absolute paths (or a bare command resolved via
  `$PATH`) in pekwm keybinds. Both binds now use `/home/jim/...`.

- **Debugging technique:** drive everything through `xdotool` + `pekwm_ctrl` against
  `DISPLAY=:0` with throwaway marked-class `kitty` windows, asserting on
  `getactivewindow` / `get_desktop_for_window`; a sentinel `echo … >> /tmp/log` at
  the top of the helper proved whether pekwm's `Exec` invoked it at all (it didn't,
  until the path was absolute). Caveat: synthetic `xdotool` modifier-drags can
  false-negative a `Motion`+`Threshold` mouse binding (the Super+right-drag resize
  tested "dead" via xdotool but works by hand).
